...
class FirestoreEventStore {
  ...
  Stream<List<Event>> watchEvents(
    ...
    final hasAccess = await hasCalendarMembership(calendarId: calendarId, uid: uid);
    if (!hasAccess) {
  ...
  String resolveDocumentId(Event event) {
    ...
    final timezone = event.timezone.isEmpty ? DateTime.now().timeZoneName : event.timezone;
    final seed = [
      event.title,
  ...
  Future<List<AppCalendar>> _fetchCalendarsByIds(List<String> calendarIds) async {
